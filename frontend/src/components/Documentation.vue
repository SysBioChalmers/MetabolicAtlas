    ...
    <div class="container is-fullhd">
      ...
      <div class="columns is-variable is-8">
        ...
        <div id="documentation" class="column content has-text-justified">
          ...
          <p>To browse our integrated GEMs, visit the <router-link :to="{ name: 'gems', hash: '#gem-repository'}">GEM Repository</router-link> page.</p>
          ...
          <p>The Metabolite page shows information on the current selected metabolite. Metabolites in GEMs are often differentiated according to their cell compartment localization (e.g., endoplasmic reticulum). For this reason, one metabolic species, e.g. cholesterol, may correspond to several different metabolite entries in a GEM, such as cholesterol[c], cholesterol[m], etc. (the suffix indicates the compartment in which the metabolite is localized).</p>
          <p>The top table contains basic information extracted from the GEM. If provided, additional identifiers from external databases will be shown in the External IDs table below.</p>
          <p>On the right side of the page, users can access the <a href="#Interaction-Partners">Interaction Partners</a> tool for the metabolite.</p>

          <h6 class="has-text-grey">Reactions table</h6>
          <p>Lists all the reactions involving the current metabolite as a reactant or a product. The current metabolite is denoted with a black text color in the reaction equations. Since metabolites are specific to a cell compartment, only reactions involving the metabolite in its specific compartment are displayed. To remove this restriction and display additional reactions involving the metabolite in any compartment, click the <i>Expand to all compartments</i> button.</p>
          ...
          <p>For a given metabolite or gene, this tool renders a connected graph of the other metabolites and genes with which it interacts via shared reaction(s). The metabolite or gene of interest is centered on the graph. Connectivity is determined based on the reactions with which this metabolite or gene is associated, i.e. a edge between two nodes maybe represente a connections that occurs in multiple reactions. For medium-sized networks of interaction partners, the user is prompted before generating the graph. For very large networks (e.g. for H<sub>2</sub>O) the graph will not be generated. Users can left-click a node to display a context menu with 3 options:</p>
          <ul>
          ...
          <p>The <i>Map Viewer</i> is - similar to the <i>Interaction Partners</i> and the <i>GEM Browser</i> - a separate tool, accessible after an integrated model has been selected. It includes a 2D viewer to vizualize metabolic maps in SVG format, and a 3D viewer to explore the metabolic network in 3 dimensions. Users can easly toggle between the <i>GEM Browser</i> and <i>Map Viewer</i> using the buttons in the top navigation bar.</p>
          <p>To switch between 2D maps and 3D network, use the "Switch to 2D" or "Switch to 3D" button in the top left of the map, respectively. This button is disabled for a model without 2D maps, or when the corresponding 2D version of a 3D network is not available. The two left sidebar buttons are used to select which compartment or subsystem will be shown in the viewer.</p>

          <h5 id="2D-Viewer" class="is-size-5">2D Viewer</h5>
          <p>2D SVG maps are provided for integrated GEMs. They represent either a cell compartment or a subsystem/pathway. While a very high percentage of the reactions in the model are represented on the 2D maps, some may be unavailable. SVG maps were produced with <a target="_blank" href="https://www.omix-visualization.com/">Omix</a>, a customizable tool for editing biochemical networks. A custom plug-in was developed to enable the creation of compact maps with consistent layout.</p>
          <p>Three buttons on the top left of the UI allow users to zoom in, zoom out and show/hide the genes on the current map.</p>
          <p>Users can interact with the maps by clicking and dragging the mouse to pan the view or using the mouse wheel to zoom in/out.</p>
          ...
          <p>The SVGs are interactive; click on a node (metabolite, reaction, gene) or a subsystem to load some of its information in the sidebar. Additonal information on the corresponding selected element can be accessed by clicking the <i>GEM Browser</i> button.</p>

          <h5 id="3D-Viewer" class="is-size-5">3D Viewer</h5>
          <p>3D renderings of the metabolic network are automatically generated from the GEM data, with the help of <a href="https://github.com/vasturiano/3d-force-graph" target="_blank">a force-directed graph JavaScript library</a>. This 3D graph contains all the reactions in the model, grouped by cellular compartment or subsystem.</p>
          <p>Interaction with the 3D graph is possible using the mouse by holding left-click and moving the mose to rotate the view, right-click to pan, and use the mouse wheel to zoom in/out.</p>
          <p>Users can also hover a node to view its name/id or left-clik on a node (once the graph has stopped moving) to display some of its information in the sidebar. Additonal information on the corresponding selected element can be accessed by clicking the <i>GEM Browser</i> button.</p>
